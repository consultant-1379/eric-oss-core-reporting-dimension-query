...
given:
  client requests to get wireless network function with no slice pofile
when:
...
""")
    request {
        ...
        urlPath('/oss-core-ws/rest/ctw/wirelessnetfunction') {
            queryParameters {
                parameter('ExternalRef::nodeFDN', 'MeContext=no-slice-profile,ManagedElement=no-slice-profile')
    ...
    response {
        ...
        body(file("wirelessnetfunction-no-slice-profile.json"))
    }
}
