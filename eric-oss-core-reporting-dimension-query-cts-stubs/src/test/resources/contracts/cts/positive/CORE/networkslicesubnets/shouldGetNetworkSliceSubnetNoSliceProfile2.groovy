...
given:
  client requests to get network slice subnet with no slice profile
when:
  a valid request is submitted
then:
  request accepted and returns network slice subnet with no slice profile

""")
    request {
        method 'GET'
        urlPath('/oss-core-ws/rest/ctw/networkslicesubnet/501') {
            queryParameters {
    ...
    response {
        ...
        body(file("networkslicesubnet-no-slice-profile-2.json"))
    }
}
