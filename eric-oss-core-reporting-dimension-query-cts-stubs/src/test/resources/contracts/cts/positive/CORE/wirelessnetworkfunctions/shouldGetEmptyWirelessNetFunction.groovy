...
given:
  client requests to get wireless network function
when:
  a valid request with invalid id is submitted
then:
  request accepted and returns an empty wireless network function list
...
""")
    request {
        ...
        urlPath('/oss-core-ws/rest/ctw/wirelessnetfunction') {
            queryParameters {
                parameter('ExternalRef::nodeFDN', regex('MeContext=(?i)[a-z0-9]+,ManagedElement=(?i)[a-z0-9]+'))
