...
given:
  client requests to get wireless network function with no service profile
when:
...
""")
    request {
        ...
        urlPath('/oss-core-ws/rest/ctw/wirelessnetfunction') {
            queryParameters {
                parameter('ExternalRef::nodeFDN', 'MeContext=no-service-profile,ManagedElement=no-service-profile')
    ...
    response {
        ...
        body(file("wirelessnetfunction-no-service-profile.json"))
    }
}
