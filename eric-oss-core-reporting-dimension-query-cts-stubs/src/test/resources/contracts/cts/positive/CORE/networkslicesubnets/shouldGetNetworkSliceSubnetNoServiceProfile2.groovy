...
given:
  client requests to get network slice subnet with no service profile
when:
  a valid request is submitted
then:
  request accepted and returns network slice subnet with no service profile

""")
    request {
        method 'GET'
        urlPath('/oss-core-ws/rest/ctw/networkslicesubnet/401') {
            queryParameters {
    ...
    response {
        ...
        body(file("networkslicesubnet-no-service-profile-2.json"))
    }
}
